...
class PessoaFisica(Cliente):
    ...
    # representação para arquivos de log
...
class Historico:
    ...
    def limite_transacoes_diarias(self):
        ...
        transacoes_diarias = [
            ...
            if datetime.strptime(transacao["Data"].split()[0], mascara) == datetime.strptime(hoje, mascara)
        ]
...
# o decorardor tem que documentar : data e hora da ação;
# nome da função; argumentos da função e o valor retornado da função;
# o nome do arquivo é log.txt, a ação não pode ser reescrita mas adicionadao ao final
# sempre numa nova linha
def log_transacao(funcao):
    def envelope():
        try:
            result = funcao()
            dia_atual = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
            txt_retorno_funcao = "falha" if result is False else "sucesso"

            with open(ROOT_PATH / "log.txt", "a", encoding="utf-8") as arquivo:
                texto_log = f"""[{dia_atual}] A operação de {funcao.__name__.capitalize()} foi executada
com {txt_retorno_funcao} e seu retorno foi {result}\n"""
...
def sacar():
    ...
    usuario = banco_bbb.procurar_cliente(cpf=cpf)
    status = True
    ...
    if usuario[0]:
        ...
        if conta[0]:
            ...
            resultado = usuario[1].realizar_transacao(conta=conta[1], transacao=saque)
            status = resultado[0]
        else:
...
def depositar():
    ...
    if usuario[0]:
        ...
        if conta[0]:
            ...
            resultado = usuario[1].realizar_transacao(conta=conta[1], transacao=deposito)
            status = resultado[0]
        else:
            print("Conta não encontrada!")
            status = False
...
def cadastrar_cliente():
    ...
    banco_bbb.adicionar_cliente(PessoaFisica(cpf=cpf, nome=nome, data_nasimento=data, endereco=endereco))
...
banco_bbb = Banco(nome="BumBle Bee")
...
menu_operacoes = f"""
    Bem vindo ao Banco {banco_bbb.nome}!
...
while True:
    opi = int(input(menu_inicial))
    match opi:
        case 1:  # Menu de usuários
            while True:
                opu = int(input(menu_usuario))
